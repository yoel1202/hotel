...
<!-- /navbar -->
 ...
 $tamano=10;
     ...
     $numero_filas = $consulta->num_rows;
             ...
             if (isset($_GET['pagina'])) {
              ...
              $pagina_inicio=($pagina-1)*$tamano;
               $consulta =$conexion->consulta("SELECT id_producir,nombre,tpr.estado,fecha,total FROM  tbl_producir tpr INNER JOIN tbl_productos as tp on fk_producto= id_producto LIMIT $pagina_inicio,$tamano");
                        }else{ 
                          $consulta =$conexion->consulta("SELECT id_producir,nombre,tpr.estado,fecha,total FROM  tbl_producir tpr INNER JOIN tbl_productos as tp on fk_producto= id_producto LIMIT 0,$tamano");
    ...
    <td><p data-placement="top" data-toggle="tooltip" title="Edit"><a  href="producir.php?comida='. encrypt($row[0]).'"  class="btn btn-primary btn-xs" ><span  class="glyphicon glyphicon-pencil"></span>Producir</a></p></td>
   ';
