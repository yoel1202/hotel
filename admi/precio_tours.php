...
<!-- /navbar -->
  ...
  <li><a href="tour.php">Tours</a></li>
    ...
    $consulta =$conexion->consulta("SELECT id_tours,nombre FROM tbl_tours order by  nombre ");
                     
                     if (isset($consulta)) {
         echo '  <div class="form-group">
       <div class="input-group">
                            <span class="input-group-addon"><span class="glyphicon glyphicon-list-alt"></span></span>
        <select id="" name="tour" class="form-control product-type">';
 ...
 <?php if (isset($GET['mensaje'])) {
  if ($GET['mensaje']=="OK") {
   ...
   # code...
                    ...
                    <input type="submit" name="guardarpreciotour" value="Guardar" class="btn btn-sm btn-primary btn-block" role="button">
  ...
  $tamano=10;
    ...
    $consulta =$conexion->consulta("SELECT id_precio_tour FROM  tbl_precio_tour as tpt  INNER JOIN tbl_tours as tt on tt.id_tours =fk_tour   ");
            $numero_filas = $consulta->num_rows;
             $total_paginas =ceil( $numero_filas / $tamano);
             if (isset($_GET['pagina'])) {
              $pagina=$_GET['pagina'];
              $pagina_inicio=($pagina-1)*$tamano;
  $consulta =$conexion->consulta("SELECT id_precio_tour,nombre,fechadesde,fechahasta,precio FROM  tbl_precio_tour as tpt  INNER JOIN tbl_tours as tt on tt.id_tours =fk_tour order by tpt.fechadesde desc  LIMIT $pagina_inicio,$tamano  ");
              }else{


    $consulta =$conexion->consulta("SELECT id_precio_tour,nombre,fechadesde,fechahasta,precio FROM  tbl_precio_tour as tpt  INNER JOIN tbl_tours as tt on tt.id_tours =fk_tour order by tpt.fechadesde desc  LIMIT 0,$tamano  ");
...
<div class="modal fade" id="edit" tabindex="-1" role="dialog" aria-labelledby="edit" aria-hidden="true">
    ...
    $consulta =$conexion->consulta("SELECT id_tours,nombre FROM tbl_tours order by  nombre  ");
       ...
       <div class="input-group">
                            <span class="input-group-addon"><span class="glyphicon glyphicon-list-alt"></span></span>
        <select id="tour" name="tour" class="form-control product-type">';
        while ($row = $conexion->extraer_registro()) {
           echo ' 
               <option value="'.encrypt($row[0]).'">'.$row[1].' </option>
          ';
  ...
  echo '</select>
        ...
        <button type="submit"  name="editarpreciotour" class="btn btn-warning btn-lg" style="width: 100%;"><span class="glyphicon glyphicon-ok-sign"></span>Actualizar</button>
      </div>
    </form>
      </div>
    ...
    <div class="modal-content">
       ...
       <div class="alert alert-danger"><span class="glyphicon glyphicon-warning-sign"></span> Esta seguro que quiere eliminar este registro?</div>
        ...
        <div class="modal-footer ">
        <button type="submit"  name="eliminarpreciotour" class="btn btn-success" ><span class="glyphicon glyphicon-ok-sign"></span>Si</button>
