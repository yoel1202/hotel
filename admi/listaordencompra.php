...
<!-- /navbar -->
  ...
  echo '</select>
     ...
     if (isset($_GET['listaorden'])) {
    # code...
  

$consulta =$conexion->consulta("SELECT id_orden,numero_orden FROM tbl_orden_compra where id_orden='".decrypt($_GET['listaorden'])."' ");
                     
                     if (isset($consulta)) {
         echo '  <div class="form-group">
       <div class="input-group">
                            <span class="input-group-addon"><span class="glyphicon glyphicon-cutlery"></span></span>
        <select id="" name="orden" class="form-control product-type">';
        while ($row = $conexion->extraer_registro()) {
           echo ' 
          <option value="'.$row[0].'">'.$row[1].'</option>
          ';
 ...
 <?php if (isset($_GET['mensaje'])) {
  if ($_GET['mensaje']=="OK") {
   ...
   # code...
                    ...
                    <input type="submit" name="guardarlistaorden" value="Guardar" class="btn btn-sm btn-primary btn-block" role="button">
                    <?php  
echo '<a  href="pdf/ordencompra.php?listaorden='.$_GET['listaorden'].'"  text="Orden de compra" class="btn btn-sm btn-primary btn-block" > Ver orden</a> ';
                    ?>
         ...
         <div class="col-md-8">
               <h4>Lista de orden de compra</h4>
...
global $conexion;
 ...
 if (isset($_GET['listaorden'])) {
   # code...
    ...
    $consulta =$conexion->consulta("SELECT id_lista,tp.nombre,numero_orden,lo.cantidad,precio,tp.unidad FROM  tbl_lista_orden lo INNER JOIN tbl_productos tp on tp.id_producto=lo.fk_producto INNER JOIN tbl_orden_compra  on id_orden=fk_orden  where fk_orden='".decrypt($_GET['listaorden'])."'");
...
$consulta =$conexion->consulta("SELECT sum(precio)  FROM tbl_lista_orden where fk_orden='".decrypt($_GET['listaorden'])."' ");
...
$consulta =$conexion->consulta("SELECT id_orden,numero_orden FROM tbl_orden_compra where id_orden='".decrypt($_GET['listaorden'])."' ");
       ...
       <div class="input-group">
                            <span class="input-group-addon"><span class="glyphicon glyphicon-cutlery"></span></span>
        <select id="orden" name="orden" class="form-control product-type">';
        while ($row = $conexion->extraer_registro()) {
           echo ' 
          <option value="'.$row[0].'">'.$row[1].'</option>
          ';
  ...
  echo '</select>
        ...
        <button type="submit"  name="editarlistaorden" class="btn btn-warning btn-lg" style="width: 100%;"><span class="glyphicon glyphicon-ok-sign"></span>Actualizar</button>
      </div>
    </form>
      </div>
    ...
    <div class="modal-content">
       ...
       <div class="alert alert-danger"><span class="glyphicon glyphicon-warning-sign"></span> Esta seguro que quiere eliminar este registro?</div>
        ...
        <div class="modal-footer ">
        <button type="submit"  name="eliminarlistaorden" class="btn btn-success" ><span class="glyphicon glyphicon-ok-sign"></span>Si</button>
