...
<!-- /navbar -->
  ...
  <li><a href="tour.php">Tours</a></li>
    ...
    $consulta =$conexion->consulta("SELECT  id_entrada,tp.nombre,te.cantidad FROM tbl_entradas te INNER JOIN tbl_productos tp on fk_producto= id_producto where te.cantidad >'0' ORDER BY nombre     ");
                     
                     if (isset($consulta)) {
         echo '  <div class="form-group">
       <div class="input-group">
                            <span class="input-group-addon"><span class="glyphicon glyphicon-list-alt"></span></span>
        <select id="" name="producto" class="form-control product-type">';
        while ($row = $conexion->extraer_registro()) {
           echo ' 
          <option value="'.encrypt($row[0]).'">'.$row[1].'    '.$row[2].'</option>
          ';
 ...
 <?php if (isset($_GET['mensaje'])) {
  if ($_GET['mensaje']=="OK") {
   echo '<div class="alert alert-success" role="alert"> Se ha insertado correctamente</div>';
    ...
    if ($_GET['mensaje']=="entrada") {
   echo '<div class="alert alert-success" role="alert"> No hay exitencias en inventario</div>';
  }
      if ($_GET['mensaje']=="mayor") {
   echo '<div class="alert alert-success" role="alert"> La cantidad es mayor a que existe en inventario</div>';
  }
   #
   # code...
 } ?>

                    <input type="submit" name="guardarsalida" value="Guardar" class="btn btn-sm btn-primary btn-block" role="button">
                     </a> </form>
                </div>
         ...
         <div class="col-md-8">
               <h4>Deducciones de productos</h4>
               <div class="row search-header" style="">
    
        <div class="col-lg-12 col-md-8 col-xs-12">
            <div id="custom-search-input">
                <div class="input-group col-md-12">
                    <input type="text" id="buscar" class="  search-query form-control" onchange="buscarsalida();" placeholder="Buscar" />
    ...
    <?php 
       ...
       global $conexion ;
 $tamano=10;
   $consulta =$conexion->consulta("SELECT ti.id_inventario,ti.cantidad,ti.fecha,ti.observacion,tl.email,tp.nombre,tp.unidad FROM  tbl_inventario as ti INNER JOIN tbl_login as tl on id_login=fk_usuario INNER JOIN tbl_productos as tp on tp.id_producto =fk_producto ");
 ...
 $numero_filas = $consulta->num_rows;
             ...
             if (isset($_GET['pagina'])) {
              ...
              $pagina_inicio=($pagina-1)*$tamano;
               ...
               $consulta =$conexion->consulta("SELECT   ti.id_inventario,ti.cantidad,ti.fecha,ti.observacion,tl.email,tp.nombre,tp.unidad FROM  tbl_inventario as ti INNER JOIN tbl_login as tl on id_login=fk_usuario INNER JOIN tbl_productos as tp on tp.id_producto =fk_producto order by ti.fecha desc LIMIT $pagina_inicio,$tamano");

              }else{

    $consulta =$conexion->consulta("SELECT   ti.id_inventario,ti.cantidad,ti.fecha,ti.observacion,tl.email,tp.nombre,tp.unidad FROM  tbl_inventario as ti INNER JOIN tbl_login as tl on id_login=fk_usuario INNER JOIN tbl_productos as tp on tp.id_producto =fk_producto  order by ti.fecha desc LIMIT 0,$tamano");
                }
    ...
    <td data-id="'.encrypt($row[0]).'">'.$row[5].'</td>';
...
echo '<ul class="pagination pull-right">
  ...
  echo '<li class=""><a href="salidas.php">1</a></li>';
for ($i=2; $i <=$total_paginas ; $i++) { 
 echo '<li class=""><a href="salidas.php?pagina='.$i.'">'.$i.'</a></li>';
}
...
<div class="modal fade" id="edit" tabindex="-1" role="dialog" aria-labelledby="edit" aria-hidden="true">
    ...
    $consulta =$conexion->consulta("SELECT id_entrada,tp.nombre,te.cantidad FROM tbl_entradas te INNER JOIN tbl_productos tp on fk_producto= id_producto  ORDER BY nombre   ");
                     
                     if (isset($consulta)) {
         echo '  <div class="form-group">
       <div class="input-group">
                            <span class="input-group-addon"><span class="glyphicon glyphicon-sort-by-alphabet"></span></span>
        <select id="producto" name="producto" class="form-control product-type">';
        while ($row = $conexion->extraer_registro()) {
           echo ' 
          <option value="'.encrypt($row[0]).'">'.$row[1].'    '.$row[2].'</option>
          ';
...
echo '     <div class="form-group">
     ...
     ?>
        ...
        <button type="submit"  name="editarsalida" class="btn btn-warning btn-lg" style="width: 100%;"><span class="glyphicon glyphicon-ok-sign"></span>Actualizar</button>
      </div>
    </form>
      </div>
    ...
    <div class="modal-content">
        ...
        <div class="form-group"  hidden="true">
                       ...
                       <div class="form-group"  hidden="true">
                        <div class="input-group">
                            ...
                            <input type="Text" class="form-control" name="cantidad" id="cantidad2" placeholder="id" />
                                   </div>
       ...
       <div class="alert alert-danger"><span class="glyphicon glyphicon-warning-sign"></span> Esta seguro que quiere eliminar este registro?</div>
        ...
        <div class="modal-footer ">
        <button type="submit"  name="eliminarsalida" class="btn btn-success" ><span class="glyphicon glyphicon-ok-sign"></span>Si</button>
