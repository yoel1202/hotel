 ...
 <?php if (isset($GET['mensaje'])) {
  if ($GET['mensaje']=="OK") {
   ...
   # code...
                    ...
                    <input type="submit" name="guardarcomida" value="Guardar" class="btn btn-sm btn-primary btn-block" role="button">
                     </a> </form>
                </div>
        ...
        <div class="col-lg-12 col-md-8 col-xs-12">
            <div id="custom-search-input">
                <div class="input-group col-md-12">
                    <input type="text" id="buscar" class="  search-query form-control" onchange="buscarcomida()" placeholder="Buscar" />
  ...
  $conexion = new Conexion();
 $tamano=10;
    $consulta =$conexion->consulta("SELECT id_producir FROM  tbl_producir INNER JOIN tbl_productos on fk_producto= id_producto ");
 ...
 $numero_filas = $consulta->num_rows;
             ...
             if (isset($_GET['pagina'])) {
              ...
              $pagina_inicio=($pagina-1)*$tamano;
               $consulta =$conexion->consulta("SELECT id_producir,nombre,tp.estado,fecha,total FROM  tbl_producir tp INNER JOIN tbl_productos on fk_producto= id_producto order by nombre desc  LIMIT $pagina_inicio,$tamano ");

              }else{
     $consulta =$conexion->consulta("SELECT id_producir,nombre,tp.estado,fecha,total FROM  tbl_producir tp INNER JOIN tbl_productos on fk_producto= id_producto order by nombre desc LIMIT 0,$tamano ");
}
                     
                     if (isset($consulta)) {
        
        while ($row = $conexion->extraer_registro()) {
          echo '  <tr  class="comidas" id="comidas">
...
if (isset($_GET['pagina'])) {
  $anterior=$pagina-1;
  if ($anterior>0) {
  echo' <li class=""><a href="comidas.php?pagina='.$anterior.'"><span class="glyphicon glyphicon-chevron-left"></span></a></li>';
  }
 
}
  echo '<li class=""><a href="comidas.php">1</a></li>';
  if ($total_paginas>7) {
  for ($i=2; $i <=7 ; $i++) { 
 echo '<li class=""><a href="comidas.php?pagina='.$i.'">'.$i.'</a></li>';
}
  }else{
...
if (isset($_GET['pagina'])) {
  ...
  if ($siguiente<=$total_paginas) {
   echo ' <li><a href="comidas.php?pagina='.$siguiente.'"><span class="glyphicon glyphicon-chevron-right"></span></a></li>';
  }
  
}else{
   echo ' <li><a href="comidas.php?pagina=2"><span class="glyphicon glyphicon-chevron-right"></span></a></li>';
}
...
echo '     <div class="form-group">
     ...
     ?>
        ...
        <button type="submit"  name="editarcomida" class="btn btn-warning btn-lg" style="width: 100%;"><span class="glyphicon glyphicon-ok-sign"></span>Actualizar</button>
      </div>
    </form>
      </div>
    ...
    <div class="modal-content">
       ...
       <div class="alert alert-danger"><span class="glyphicon glyphicon-warning-sign"></span> Esta seguro que quiere eliminar este registro?</div>
        ...
        <div class="modal-footer ">
        <button type="submit"  name="eliminarcomida" class="btn btn-success" ><span class="glyphicon glyphicon-ok-sign"></span>Si</button>
